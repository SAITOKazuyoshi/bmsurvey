...
class XoopsObjectGenericHandler extends XoopsObjectHandler
{
	 ...
	 * @return null|void
	 */
	private function &_getCriteria($id){
		// Bluemoon hack start
		$i = 0;
		if (is_array($id)){
	...
	function _update(&$obj) {
		...
		$arr = $this->_makeVars4sql($obj);
		// Bluemoon hack start
		if (preg_match('/,/',$this->mPrimary)){
			$pKeys = explode(",",$this->mPrimary);
		}else{
			$pKeys = array($this->mPrimary);
		}
		foreach ($arr as $_name => $_value) {
	...
	*/
	 ...
	 * @return bool
	 */
	private function &_deleteCriteria($obj){
		// Bluemoon hack start
		$mPrimary = explode(",",$this->mPrimary);
		$criteria = new CriteriaCompo();
		foreach($mPrimary as $key){
			$criteria->add(new Criteria($key,$obj->getVar($key)));
		}
		return $criteria;
		// Bluemoon hack end
	}
	...
	function delete(&$obj, $force = false)
	{
		...
		$criteria = $this->_deleteCriteria($obj);
		//
