...
class Model_Post extends AbstractModel{
	...
	private function _checkPostType($questionObject,$value){
		...
		if ( strlen($value) > 0 ) {
			switch ( $questionObject->getVar('type_id') ) {
				case 4: // single
				case 5: // Multiple
					$qName_other = "Q".$questionObject->getVar('id')."_".$value;
					if (isset($_POST[$qName_other])){
						$otherValue = array(
							"choice_id" => $value,
							"value" => $this->getRequest( $qName_other )
						);
					}
					break;
				case 9: // Date
	...
	private function &_checkPost(&$questionObject,$value){
		...
		$otherValue = $this->_checkPostType($questionObject,$value);
		if (!$this->errorGuide){
			$this->passed = true;
		}
		return $otherValue;
	...
	public function getPost(){
		foreach($this->questionObjects as $questionObject){
			...
			if (isset($_POST[$qName])){
				$otherValue = $this->_checkPost( $questionObject, $this->getRequest( $qName ));
	...
	private function &_getObject(&$handler,$response_id,$question_id,$choice_id=null){
		$criteria = new CriteriaCompo();
		$criteria->add(new Criteria('response_id',$response_id));
		$criteria->add(new Criteria('question_id',$question_id));
		if ($choice_id){
		...
		if ($responseObjects){
			...
			if ($choice_id){
				$object->set('choice_id',$choice_id);
			}
		}
		return $object;
	}
	private function _response_single(&$handler,$response_id,$question_id,$value){
	...
	private function _response_multi(&$handler,$response_id,$question_id,$value){
		foreach($value as $v){
			...
			$object->set('response_id',$response_id);
			$object->set('question_id',$question_id);
			$object->set('choice_id',$v);
			$handler->insert($object);
		}
	}
	private function _response_other(&$handler,$response_id,$question_id,$value){
		if(!is_array($value)) return null;
		$object = $this->_getObject($handler,$response_id,$question_id,$value['choice_id']);
		$object->set('response',$value['value']);
		$handler->insert($object);
...
//		$responseRankHandler = xoops_getmodulehandler('response_rank');
		...
		foreach($this->questionObjects as $questionObject){
			$question_id = $questionObject->getVar('id');
			$value = $this->receiveArray[$question_id]['postValue'];
			switch ( $questionObject->getVar('type_id') ) {
				case 1: // Yes/No
					$this->_response_single($responseBoolHandler,$response_id,$question_id,$value);
					break;
				case 2: // Text
				...
				case 3: // Text Area
					...
					break;
				case 4: // Radio button
					$otherValue = $this->receiveArray[$question_id]['otherValue'];
					$choice_id = $this->_response_other($responseOtherHandler,$response_id,$question_id,$otherValue);
					if ($choice_id){
						$value = $choice_id;
					}
					$this->_response_single($responseSingleHandler,$response_id,$question_id,$value);
					break;
				case 5: // Check box
					...
					$otherValue = $this->receiveArray[$question_id]['otherValue'];
					$this->_response_other($responseOtherHandler,$response_id,$question_id,$otherValue);
					break;
