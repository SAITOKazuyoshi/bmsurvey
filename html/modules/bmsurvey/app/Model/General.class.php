...
class Model_General
{
	...
	public function setStatus($form_id, $status)
	{
		...
		$this->formObject = $this->mHandler->get($form_id);
		if($this->formObject){
			if ($this->formObject->getVar('status')==STATUS_TEST){
				// Delete test data on response table
				$Model_Question = Model_Question::forge();
				$questionObjects = $Model_Question->getObjectsOnForm($form_id);
				$Model_Response = Model_Response::forge();
				$Model_Response->delete($questionObjects,$form_id);
			}
