...
class View_Question extends View
{
	...
	public function &generateForm(&$htmlRender,&$questionObjects,&$valueArray=null){
		$Model_Question = Model_Question::forge();
		$tags = array();
		foreach($questionObjects as $questionObject){
			$choiceObjects = $Model_Question->getChoice( $questionObject->getVar('id') );
			$tags[] = $htmlRender->generateHtml($questionObject,$choiceObjects,$valueArray);
		}
		return $tags;
	}

	public function &mixinReport(&$htmlRender,&$questionObjects,&$valueArray=null){
		$Model_Question = Model_Question::forge();
		$tags = array();
		foreach($questionObjects as $questionObject){
			$choiceObjects = $Model_Question->getChoice( $questionObject->getVar('id') );
			$tags[] = $htmlRender->mixinReport($questionObject,$choiceObjects,$valueArray);
		}
		return $tags;
	}
	...
	public function &setQuestions(&$htmlRender,&$questionObjects,&$currentObject,&$choiceObjects)
	{
		...
		return array(
			...
			'preview' => $this->generateForm($htmlRender,$questionObjects),
			...
			'edit' => array(
				...
				'required' => array(
					'tag' => $htmlRender->mkselect("required", array("Y" => _MB_Yes, "N" => _MB_No), $currentObject->getVar("required"), 'N'),
					'label' => _MB_Required
				),
			),
